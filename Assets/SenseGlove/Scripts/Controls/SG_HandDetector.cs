...
namespace SG
{
    ...
    public class SG_HandDetector : MonoBehaviour
    {
        ...
        /// <summary> Contains detection arguments for a single glove </summary>
        public class HandDetectArgs
        {
            //parameters

            public SG_SenseGloveHardware Glove
            {
                get; private set;
            }
            ...
            public HandDetectArgs(SG_SenseGloveHardware detectedGlove)
            {
                Glove = detectedGlove;
                EventFired = false;
                DetectionTime = 0;
        ...
        protected virtual void LateUpdate()
        {
            for (int i = 0; i < this.detectedHands.Count; i++)
            {
                if (this.detectedHands[i].TotalColliders >= this.activationThreshold)
                {
                    ...
                    {
                        this.FireDetectEvent(this.detectedHands[i].Glove);
        ...
        void OnTriggerEnter(Collider col)
        {
            ...
            if (touch && touch.linkedGlove) //needs to have a grabscript attached.
            {
                ...
                if (this.activationTime <= 0 && this.detectedHands[scriptIndex].TotalColliders == this.activationThreshold)
                {
                    ...
                    if (!(detectedHands[scriptIndex].EventFired) && !(this.singleGlove && this.detectedHands.Count > 1))
                    {
                        ...
                        this.FireDetectEvent(this.detectedHands[scriptIndex].Glove);
                    }
                }
                ...
                {   //belongs to an existing SenseGlove.
                    if (ValidScript(touch))
                    {
                        ...
                        if (this.detectedHands[scriptIndex].TotalColliders <= 0)
                        {
                            ...
                            {
                                this.FireRemoveEvent(this.detectedHands[scriptIndex].Glove);
                            }
        ...
        private int HandModelIndex(SG_SenseGloveHardware model)
        {
            for (int i = 0; i < this.detectedHands.Count; i++)
            {
                if (GameObject.ReferenceEquals(model.gameObject, this.detectedHands[i].Glove.gameObject)) { return i; }
            }
            return -1;
        }
        ...
        {
            SG_SenseGloveHardware[] res = new SG_SenseGloveHardware[this.detectedHands.Count];
            for (int i=0; i<this.detectedHands.Count; i++)
            {
